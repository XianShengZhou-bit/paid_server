...
using ClientHandler = std::function<void(int client_fd, const std::string& client_ip)>;
        ...
        while (running == nullptr || running->load()) {
            ...
            for (int i = 0; i < ready; ++i) {
                ...
                while (running == nullptr || running->load()) {
                    ...
                    const std::string peer_ip = client_ip;
                    setReadTimeout(client_fd);

                    gateway_http::WorkerPool::instance().submit([this, client_fd, peer_ip, handler]() {
                        try {
                            handler(client_fd, peer_ip);
                        } catch (const std::exception& ex) {
                            LOG_ERROR("HTTP 连接处理异常: client_ip={}, error={}", peer_ip, ex.what());
                        } catch (...) {
                            LOG_ERROR("HTTP 连接处理未知异常: client_ip={}", peer_ip);
                        }
                        ::close(client_fd);
