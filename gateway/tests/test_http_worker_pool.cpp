...
/*
测试：gateway_http::WorkerPool
1. 未 init 时 submit 抛异常
2. init 后 submit 能异步执行任务
3. init(1) 等过小 worker 数会被抬升到至少 4
4. initFromConfig / httpServer() / wsMaxConnections 读取 .env
5. 重复 init 幂等
*/

// review
TEST(GatewayHttpWorkerPoolTest, LifecycleSubmitAndConfig) {
    auto& pool = gateway_http::WorkerPool::instance();
    ...
    pool.submit([&promise]() { promise.set_value(7); });
    ...
    EXPECT_EQ(payment_config::Config::instance().wsMaxConnections(), 64);
