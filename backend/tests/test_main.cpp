...
// 从 argv 取出 env 路径（第一个非 '-' 开头的参数），并从参数列表中移除。
// gtest_discover_tests 探测时命令为: payment_tests --gtest_list_tests <env>
// 手跑 / ctest 执行时为: payment_tests <env> [--gtest_filter=...]
// review
...
int main(int argc, char** argv) {
    ...
    if (env_path.empty()) {
        std::cerr << "用法: " << (argc > 0 ? argv[0] : "payment_tests") << " <env文件路径> [gtest选项...]\n"
